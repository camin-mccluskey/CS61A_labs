...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    city_at_lat_lon = make_city('temp',lat,lon)
    distance1 = distance(city1, city_at_lat_lon)
    distance2 = distance(city2,city_at_lat_lon)
...
def put_piece(board, max_rows, column, player):
    ...
    -1
    """
    # check bottom row for given column then move up if full, else replace
    row = max_rows - 1
    while row >= 0:
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    -1
    """
    # check col is a valid column number
